...
- [x] **RF-002** O sistema deve aceitar upload de arquivo de vídeo (MP4, AVI, MOV) com tamanho máximo de 2 GB
- [x] **RF-003** O sistema deve aceitar upload de arquivo de áudio (MP3, WAV, OGG) com tamanho máximo de 500 MB
...
- [x] **RF-007** O sistema deve detectar expressões faciais de dor, medo e sofrimento utilizando DeepFace
- [x] **RF-008** O sistema deve analisar a postura corporal da paciente para detectar sinais de coerção ou desconforto (MediaPipe)
- [x] **RF-009** O sistema deve detectar objetos de risco no ambiente clínico utilizando YOLOv8
...
- [x] **RF-016** O sistema deve detectar o idioma predominante no áudio automaticamente
...
- [x] **RF-053** O sistema deve mascarar dados sensíveis de pacientes (anonimização: ex. Maria Silva → PACIENTE_001) com base no perfil de acesso
- [x] **RF-054** O sistema deve rotear toda comunicação de mídia com a nuvem pelo AWS Integration Domain para garantir conformidade

---

## 2. Requisitos Não Funcionais

- [x] **RNF-001** Performance: O sistema deve processar um vídeo de 30 minutos em no máximo 15 minutos
- [x] **RNF-002** Performance: A API Gateway deve responder em menos de 500ms para requisições síncronas
- [x] **RNF-003** Performance: O dashboard deve carregar em menos de 3 segundos
- [x] **RNF-004** Disponibilidade: O sistema deve ter disponibilidade mínima de 99% em ambiente de produção na nuvem (PRD)
- [x] **RNF-005** Escalabilidade: Cada serviço de domínio deve escalar horizontalmente de forma independente no ECS Fargate (em HML/PRD)
- [x] **RNF-006** Segurança (Trânsito): Todos os dados em trânsito devem ser criptografados via HTTPS/TLS 1.3
- [x] **RNF-007** Segurança (Repouso): Dados de pacientes e mídias devem ser criptografados em repouso com algoritmo AES-256 (via Amazon S3 Encryption e DB Encryption)
- [x] **RNF-008** Segurança (Identidade): A Gestão de Segredos e credenciais deve ser feita exclusivamente via AWS Secrets Manager e IAM policies
- [x] **RNF-009** Segurança (LGPD): O sistema deve estar em estrita conformidade com a LGPD (Lei 13.709/2018), garantindo minimização, anonimização e privacidade por design
- [x] **RNF-010** Qualidade: A cobertura de testes unitários deve ser ≥ 80% por serviço
- [x] **RNF-011** Qualidade: O código deve passar no linting (ruff/flake8) sem erros
...
- [x] **RNF-014** Manutenibilidade: Toda API deve ter documentação OpenAPI/Swagger atualizada
...
- [x] **RNF-017** Usabilidade: O dashboard deve ser responsivo e funcionar em resolução mínima de 1280x720
- [x] **RNF-018** Auditabilidade: Todo acesso a dados de pacientes deve ser registrado em log imutável no CloudWatch Logs
...
- [x] **RN-008** LGPD: Dados de pacientes devem ser completamente anonimizados/pseudo-anonimizados nas bases analíticas e mantidos em segredo de acordo com o CFM
- [x] **RN-009** Documentos: O consentimento informado deve ser verificado antes do processamento de qualquer mídia
- [x] **RN-010** Vídeo: A análise de vídeo deve ser realizada apenas em arquivos previamente autorizados pelo consentimento verificado
- [x] **RN-011** Áudio: A gravação de consultas requer consentimento explícito documentado e registrado no sistema
- [x] **RN-012** Papéis: Médicos e Enfermeiros podem criar sessões e visualizar seus pacientes; Auditores e Admins podem acessar logs e relatórios anonimizados
