...
**Atenção:** Em versões futuras (v3.0), as colunas legadas `ira_score`, `score_video`, etc., serão deletadas via `op.drop_column('sessions', 'ira_score')`. Não crie novas regras de negócio lendo dessas colunas, consuma a rota `/api/session/{id}/risk-summary`.
